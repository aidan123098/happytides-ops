...
import type { DashboardSnapshot } from "@/lib/live-metrics";

type PriorityTone = "blue" | "green" | "amber" | "slate";

export function DashboardWorkspace({ initialDashboard }: { initialDashboard: DashboardSnapshot }) {
  const [dashboard, setDashboard] = useState(initialDashboard);
  const { metrics, products, revenueSeries, inventoryHealth, recentOrders, topCustomers, paidOrderCount } = dashboard;
  const todayLabel = useMemo(() => new Date().toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" }), []);
  const hotProducts = useMemo(() => [...products].sort((left, right) => right.unitsSoldWeek - left.unitsSoldWeek).slice(0, 6), [products]);
  ...
  useLiveRefresh({
    onRefresh: async () => {
      ...
      if (payload?.metrics && Array.isArray(payload.recentOrders)) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.85fr)] lg:items-center lg:p-6">
          <div className="min-w-0">
            ...
            <div className="mt-4 max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Track sales, order flow, stock risk, and customer momentum.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="rounded-md border border-white/10 bg-white/[0.06] p-3">
              ...
              <div className="mt-2 text-xl font-semibold sm:text-2xl">{formatNumber(inventoryHealth.totalOnHand)}</div>
              <div className="mt-1 text-[11px] leading-4 text-slate-400">{formatNumber(inventoryHealth.reserved)} reserved</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-3 sm:gap-4 2xl:grid-cols-4">
        ...
        <MetricCard title="Inventory risk" value={formatNumber(inventoryHealth.lowStock.length)} detail={`${formatNumber(inventoryHealth.reserved)} units reserved`} icon={AlertTriangle} tone={inventoryHealth.lowStock.length > 0 ? "amber" : "green"} />
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <Card>
          <CardHeader>
            ...
            <Badge tone="slate">{formatNumber(paidOrderCount)} paid orders</Badge>
          </CardHeader>
          <CardContent className="space-y-3">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {[
          { title: "Fulfillment stance", detail: inventoryHealth.lowStock.length > 0 ? "Inventory needs active attention before the next rush." : "Stock coverage looks workable from current thresholds.", icon: Truck, tone: inventoryHealth.lowStock.length > 0 ? "amber" : "green", href: "/inventory" },
